...
def html_report_valtest_global_drift(res: dict, semaphore_title: str) -> str:
    ...
    return render_test_report(
        ...
        "При резервном отборе прогноз рассчитывается по признакам с наибольшей корреляцией; "
        "статистическая значимость их связи с качеством не подтверждена.",
        ...
        "Если отбор пуст, применяется резервный выбор по |r|: "
        "не более одного признака на четыре подвыборки. Это отступление от строгого отбора по методике.",
        "Пороги по умолчанию: зелёный — D < 0,15 и зелёная КМ на эталоне; красный — "
        "D ≥ 0,25 или красная КМ на эталоне; иначе жёлтый. Серый: корреляции не определены, "
