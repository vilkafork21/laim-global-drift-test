...
## Статус теста: экспериментальный (карточка 6.3.8)

Методика относит прогноз влияния сдвига к экспериментальным тестам: его
точность должна быть подтверждена на истории размеченных периодов решения, а
без такой истории результат — «не оценено». Поэтому нода **всегда** публикует
`status = not_computable`, `color = gray`, `reason_code = no_labeled_history`,
`informative = true`; светофор итерации она не формирует, агрегатор учитывает
её как информативную. Диагностика (метрика эталона и мониторинга по
`scored_data`, регрессионная оценка `drift_metric_value_estimate`, отобранные
признаки, `selection_low_confidence`, `n_oos`/`n_oot`) сохраняется в
`all_results` и HTML для расследования и будущей калибровки. Прежний цвет по
снижению среднего оценок судьи снят: он дублировал тест динамики КМ.
